...
use tari_cc_private_ballot_ootle_anchor_app::{AnchorEvidenceRecordV1, evidence::MAX_EVIDENCE_FILE_BYTES};
...
pub fn verify_transport_archive_anchor_v1(
    ...
    if !result.archive_verified || !result.transport_binding_verified {
        return Ok(result);
    }
    ...
    let evidence_bytes = std::fs::read(evidence_path)
        .map_err(|_| GuiCoreError::io_failure("anchor-evidence"))?;
