...
use tari_cc_private_ballot_protocol::{
    CanonicalCborReader, CanonicalCborWriter, HashDomain, MAX_CANONICAL_OBJECT_BYTES, ManifestHash,
    ProtocolError, ValidationCode, hash_domain_separated,
};
...
impl TransportArchiveBatchV1 {
    ...
    #[must_use]
    pub const fn accepted_unique_count(&self) -> u64 {
...
impl TransportArchiveBindingV1 {
    pub fn new(
        ...
        if batches.is_empty() || batches.len() > MAX_BATCHES {
            return Err(invalid(
                "transport binding must contain a bounded non-empty batch set",
            ));
        }
    ...
    #[must_use]
    pub const fn descriptor_generation(&self) -> u64 {
        self.descriptor_generation
    }
    #[must_use]
    pub fn batches(&self) -> &[TransportArchiveBatchV1] {
        &self.batches
    }
    #[must_use]
    pub const fn final_batch_set_commitment(&self) -> [u8; 32] {
        self.final_batch_set_commitment
    }
    ...
    pub fn from_canonical_cbor(bytes: &[u8]) -> Result<Self, ProtocolError> {
        ...
        for _ in 0..count {
            if reader.read_array_len()? != BATCH_FIELD_COUNT {
                return Err(invalid(
                    "transport binding batch has an invalid field count",
                ));
            }
...
fn read_fixed<const N: usize>(
    ...
    <[u8; N]>::try_from(reader.read_byte_string()?)
        .map_err(|_| invalid("transport binding digest length is invalid"))
}
...
mod tests {
    ...
    fn canonical_round_trip_sorts_by_sealed_batch_identity() {
        ...
        let decoded =
            TransportArchiveBindingV1::from_canonical_cbor(&bytes).expect("canonical decode");
